...
Run `corepack enable && pnpm install --frozen-lockfile` before the Node-backed skill guards. `ci.sh` is the aggregate gate — run `bash scripts/ci.sh` before every commit (manifests + version lockstep, structural guards, score floors, content-hash idempotency). The bundled validators default their root to `plugins/docks/skills`, so always pass the explicit `plugins/effect-kit/skills` path.
